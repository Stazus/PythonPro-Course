"""
Program: Automatyczne testowanie wyjątków FileNotToundError i PermissionError

Opis działania i sposób testowanjia:
1. Uruchom program w terminalu:
    python3 task3_czytanie_pliku.py
    
2. Po uruchomieniu program zapyta, który test chcesz wykonać:
    - Wpisz 1 -> test FileNotFounder (plik nie istnieje)
    - Wpisz 2 -> test PermissionError (plik istnieje, ale brak uprawnień do odczytu)
    - Wpisz 3 -> test normalnego odczytu (plik istnieje i ma prawo odczytu)
    
3. Po teście PermissionErr program automatycznie przywróci uprawnienia pliku.
"""
...
def czytaj_plik(nazwa_pliku):
    """ 
    Funkcja próbuje otworzyć i odczytac plik o podanej nazwie.
    Obsługuje wyjatki:
    ...
    try:
        ...
        # Wyjatek, gdy brak uprawnień do odczytu
        print(f"Błąd: Brak uprawnień do odczytu pliku '{nazwa_pliku}'.")
        
    except Exception as e:
        # Obsługa innych, nieoczekiwanych błądów
