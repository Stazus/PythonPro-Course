...
class Query(graphene.ObjectType):
    ...
    search_users = graphene.List(User, name=graphene.String())

    def resolve_users(root, info):
        return users_data

    def resolve_posts(root, info, author_id=None):
        if author_id is None:
            return posts_data

        return [
            post for post in posts_data
            if post["author_id"] == int(author_id)
        ]

    def resolve_search_users(root, info, name=None):
        if not name:
            return users_data
